            ...
            <article key={order.id} className="grid gap-4 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <a
                    ...
                    className="mt-1 block break-all text-sm text-rose-700"
                  >
                    {order.phone}
                  </a>
                  {order.email ? (
                    <a
                      ...
                      className="mt-1 block break-all text-sm text-stone-600"
                    >
                      {order.email}
                    </a>
                  ) : null}
                ...
                {items.map((item) => (
                  ...
                  >
                    ...
                    <p className="shrink-0 text-stone-700">
                      {formatPrice(item.unitPriceMinor, "ru")}
                    </p>
                  </div>
                ))}
              </div>
              ...
              >
                ...
                <select
                  ...
                  className="h-10 min-w-0 rounded-md border border-stone-300 bg-white px-2 outline-none focus:border-rose-700"
                >
                ...
                <button
                  type="submit"
                  className="h-10 rounded-md bg-stone-950 px-3 text-xs font-medium text-white hover:bg-stone-800"
                >
                  Сохранить
                </button>
              </form>
            ...
            {orders.map((order) => {
              ...
              return (
                <tr key={order.id}>
                  ...
                  <td className="max-w-xs whitespace-normal px-4 py-3 text-stone-700">
                    {order.deliveryAddress || "—"}
                  </td>
                  <td className="px-4 py-3 text-stone-700">
                    <form
                    ...
                    >
                      ...
                      <select
                        ...
                        defaultValue={order.status}
                        aria-label={`Статус заказа ${order.id}`}
                        className="h-9 rounded-md border border-stone-300 bg-white px-2 outline-none focus:border-rose-700"
                      >
                        {Object.entries(statusLabels).map(([value, label]) => (
                          <option key={value} value={value}>
                            {label}
                      ...
                      <button
                        type="submit"
                        className="h-9 rounded-md bg-stone-950 px-3 text-xs font-medium text-white hover:bg-stone-800"
                      >
                        Сохранить
                      </button>
                    </form>
