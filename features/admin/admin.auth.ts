...
export async function authenticateAdmin(
  ...
  staleAuditCutoff.setUTCDate(
    staleAuditCutoff.getUTCDate() - LOGIN_AUDIT_RETENTION_DAYS,
  );
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (oldestSessions.length >= MAX_ACTIVE_ADMIN_SESSIONS) {
        ...
        await tx.adminSession.deleteMany({
          where: {
            tokenHash: {
              in: sessionsToDelete.map((session) => session.tokenHash),
            },
          },
        });
      }
      ...
      await tx.adminLoginEvent.deleteMany({
        where: { createdAt: { lt: staleAuditCutoff } },
      });
    });
